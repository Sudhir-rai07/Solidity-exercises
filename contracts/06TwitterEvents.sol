...
contract TwitterStruct {
    ...
    // Create event fotr creating tweet
    // Params : id, author, content, timestamp
    // 
    // Emit the event in createTweet() functiom
    event TweetCreated(uint256 _id, address _author, string _content, uint256 _timestamp);

    // Create a event for liking a tweet
    // Params : liker, tweetId, likeCount
    event TweetLiked(address liker, uint256 _Tid, uint256 _likeCount);
    ...
    function likeTweet(address author, uint256 id) external {
        ...
        // emit TweetLiked event
        emit TweetLiked(msg.sender, id, tweets[author][id].likes);
    }
